...
# Config
# =========================
EVE_JSON_FILE = "logs/eve.json"   # real or synthetic
RESULTS_DIR = "results"
...
def generate_event():
    ...
    src_ip = random.choice(MALICIOUS_IPS + BENIGN_IPS)
    dest_ip = random.choice(MALICIOUS_IPS + BENIGN_IPS)
    ...
    dns = {}
    if event_type=="dns":
        domain = random.choice(MALICIOUS_DOMAINS + BENIGN_DOMAINS)
        dns = {"rrname": domain,
...
def ioc_match(event):
    ...
    if event.get("event_type")=="dns":
        if event.get("dns",{}).get("rrname","") in MALICIOUS_DOMAINS:
            hits += 1
    return hits
...
# =========================
# Real-time detection (on EVE_JSON_FILE)
# =========================
def follow(file):
        ...
        with open(EVE_JSON_FILE) as log:
            for line in log:
                ...
                if hits>0 or ml_score>THREAT_THRESHOLD:
                    alerts += 1
                    writer.writerow([event.get("timestamp"),event.get("src_ip"),event.get("dest_ip"),hits,round(ml_score,3)])
                    out.flush()
                    print(f"[ALERT] {event.get('timestamp')} | SRC={event.get('src_ip')} → DST={event.get('dest_ip')} | IOC={hits} | ML={ml_score:.2f}")
                if total%100==0:
